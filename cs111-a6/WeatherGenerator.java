...
public class WeatherGenerator {
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) 
    {
        ...
        //StdRandom.setSeed(1668046361875L);
        ...
        {
            double newProb = StdRandom.uniform(0.0, 1.0);
            if(forecast[i-1] == 1)
            {
                if(newProb <= wetwetProbability)
                {
                    forecast[i] = 1;
                }
                else 
            ...
            else if(newProb <= drywetProbability)
            {
                forecast[i] = 1;
            }
            else 
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude )
    {
        ...
        int[] oneMonthForecast = forecastGenerator(drywetProbability[month], wetwetProbability[month], numberOfDaysInMonth[month]);
