public class LeasingCost 
{
	...
	public static Vehicle createVehicle(String description)
    {
        ...
        int allowance = Integer.parseInt(description.substring(description.indexOf("allowance:") + 10 , description.indexOf(";",description.indexOf("allowance:"))));
        double charger = 0.0;
        ...
        {
            charger = Double.parseDouble(description.substring(description.indexOf("charger:") + 8 , description.indexOf(";",description.indexOf("charger:"))));;
            f1 = new Fuel(usage, charger);
        }
        Lease l1 = new Lease(due, length, monthly, allowance);

        Vehicle car = new Vehicle(name, f1, l1);
    ...
    public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice )
    {
        ...
        for (int i = 0; i < vehicles.length; i++) 
        {
            ...
            {
                co2Emission = computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), vehicles[i].getFuel().CO2EMITTED_GASCOMBUSTION);
                totalFuelCost = computeFuelCost(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), gasPrice);
                totalLeaseCost = computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), vehicles[i].getLease().getLeaseLength(), vehicles[i].getLease().getMonthlyCost());
                totalCost = totalFuelCost + totalLeaseCost;
            }
            else if (vehicles[i].getFuel().getType() == 2) 
            {
                co2Emission = computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), vehicles[i].getFuel().CO2EMITTED_ELECTRICITYCOMBUSTION);
                totalFuelCost = computeFuelCost(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), electricityPrice);
                totalLeaseCost = computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), vehicles[i].getLease().getLeaseLength(), vehicles[i].getLease().getMonthlyCost());
                totalCost = totalFuelCost + totalLeaseCost + vehicles[i].getFuel().getCharger();
